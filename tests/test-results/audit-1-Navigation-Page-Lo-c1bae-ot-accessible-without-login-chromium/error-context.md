...
- Name: audit.spec.js >> 1. Navigation & Page Load >> 1.7 /session/* not accessible without login
- Location: audit.spec.js:91:3

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:5173/session/999
Call log:
  - navigating to "http://localhost:5173/session/999", waiting until "load"
...
> 92  |     await page.goto('/session/999');
  ...
  181 |     await page.click('button:has-text("Student")');
  182 |     await page.fill('input[placeholder="Jane"]', 'Test');
  183 |     await page.fill('input[placeholder="Doe"]', 'User');
  184 |     await page.click('button:has-text("Continue")');
  185 |     // Enter too short email and weak password
  186 |     await page.fill('input[type="email"]', 'test@test.com');
  187 |     await page.fill('input[type="password"]', 'short');
  188 |     await page.click('button:has-text("Continue")');
  189 |     await expect(page.locator('text=Please meet all password requirements')).toBeVisible();
  190 |   });
  191 | 
  192 |   test('3.5 Cannot register with already-taken email', async ({ page }) => {
```
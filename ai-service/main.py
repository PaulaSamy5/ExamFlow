...
def evaluate_math_logic(req: MathRequest) -> EvaluationResult:
    ...
    def normalize_math(text):
        ...
        # Remove common math prefixes like "x=" or "result:"
        text = re.sub(r'^[a-z]=', '', text)
    ...
    def is_numerically_equal(s1, s2, tol):
        ...
        if s1_norm == s2_norm: return True
        
        try:
            safe_chars = set("0123456789./*-+()e")
            s1_clean = "".join(c for c in s1_norm if c in safe_chars)
            s2_clean = "".join(c for c in s2_norm if c in safe_chars)
            
            if s1_clean and s2_clean:
                if re.match(r'^[0-9./*\-+()e]+$', s1_clean) and re.match(r'^[0-9./*\-+()e]+$', s2_clean):
                    try:
                        v1 = eval(s1_clean)
                        v2 = eval(s2_clean)
    ...
    final_correct = is_numerically_equal(req.studentAnswer, req.modelAnswer, req.tolerance or 0.01)
